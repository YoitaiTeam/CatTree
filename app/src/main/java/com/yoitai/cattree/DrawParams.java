...
public class DrawParams {
    ...
    public DrawParams() {
        mPos = new Vector2();
        mScl = new Vector2();
        mOfs = new Vector2(100.0f, 100.0f);
        mColor = new Color();
    }
    ...
    public void runSprite(MainRenderer _renderer) {
        ...
        // 各座標セット
        sPosWork[0].Set(0.0f, tex.getSize().Y * (1.0f - mOfs.Y / 100.0f));
        sPosWork[1].Set(tex.getSize().X, tex.getSize().Y * (1.0f - mOfs.Y / 100.0f));
        sPosWork[2].Set(0.0f, tex.getSize().Y);
        sPosWork[3].Set(tex.getSize().X, tex.getSize().Y);

        // 各テクスチャ座標セット
        sTexelWork[0].Set(0.0f, (tex.getSize().Y * (1.0f - mOfs.Y / 100.0f)) / tex.mTexSize.Y);
        sTexelWork[1].Set(tex.getSize().X / tex.mTexSize.X, (tex.getSize().Y * (1.0f - mOfs.Y / 100.0f)) / tex.mTexSize.Y);
...
//		GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
    ...
    public void run(MainRenderer _renderer) {
        switch (mType) {
            case TYPE_SCREENCLEAR:
                runScreenClear(_renderer);
                break;        // スクリーンクリア
            case TYPE_SPRITE:
                runSprite(_renderer);
                break;        // スプライト描画
